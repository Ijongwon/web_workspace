...
<body>
	...
	%>
	${name }
	<!-- 일반 자바변수는 애트리뷰트가 아니므로 출력이 안됩니다. -->
	<h3>core(c로 시작하는 태그) 예시</h3>
	<!-- page scope의 애트리뷰트 저장 -->
	<c:set var="age" value="10" />
	age : ${age}
	<br>
	<!-- 일반적으로 el 출력은 c:out 태그로 합니다. (보안.웹취약성 문제) -->
	<hr>
	<!-- c:if 조건식은 test 속성에서 el로 작성합니다. -->
	<c:if test="${age<20 }">
	...
	<!-- for 시작값은 1, 마지막값은 10, 변수명은 var로 지정. step은 증가값. 기본값은 1 -->
		<c:out value="${i*10}" />
		<br>
...
<!-- fmt -->
   ...
   <!-- 2022-11-03 20:11:00 시간이 24시 기준 -->
   <fmt:formatDate value="${datetime1 }" pattern="yyyy-MM-dd a hh:MM:ss"/><br>
   <!-- 2022-11-03 08:11:00  시간이 12시 기준(hh). a는 오전/오휴 표시 -->
