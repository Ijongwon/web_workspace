...
public class MemberDao {
	...
	public int update(MemberVo member) throws SQLException {
		String sql ="update member_tbl_02  \r\n"
				+ "set phone = ?, address = ? , city=? , grade=?, custname = ? \r\n"
				+ "where custno = ?";
		Connection conn = OracleUtil.getConnection();
		PreparedStatement pstmt = conn.prepareStatement(sql);
		...
		pstmt.setString(5, member.getCustname());
...
//		conn.rollback();    //하나의 트랜잭션을 구성하는 여러 SQL 중 일부 오류일때. 이미실행된 SQL은 취소합니다.
	...
	public int nextSeq() throws SQLException {
		...
		String sql ="select max(custno)+1 from MEMBER_TBL_02";   
					//자동발생 custno 화면표시하고 insert 할 때 실제 사용
		PreparedStatement pstmt = conn.prepareStatement(sql);
...
//  		+ "VALUES (seq_custno.nextval, ?, ?, ?, sysdate, ?, ?)";
		...
		pstmt.setString(2, member.getCustname());		//1 : 인덱스값 . ? 기호 순서
	...
	public List<MemberVo> selectList() throws SQLException {		//Member 객체를 저장한 리스트를 리턴
		...
		List<MemberVo> memberList = new ArrayList<>();		//select 조회결과를 자바 객체로 저장.
		while(rs.next()) {
	...
	public MemberVo selectOne(int custno) throws SQLException {
