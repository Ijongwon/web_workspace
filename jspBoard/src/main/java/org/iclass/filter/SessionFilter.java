...
//*.jsp 와 같은 url 형식인 모든 요청에 대헤 처리될 필터 입니다. 목적 : session 검사하기
...
public class SessionFilter extends HttpFilter implements Filter {     
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain) 
		...
		//getServletPath() 는 무엇일까요 : contextPath 뒤에 나오는 url 요청주소
		...
		BookUser user = (BookUser) session.getAttribute("user");		//로그인 여부확인을 위해 가져오기
		
		
		if(nossesions.contains(request.getServletPath()) && user != null) {
			//요청 경로가 nosessions 중에 하나이고 로그인이 되었다면 접근 못함.
		...
		if(logins.contains(request.getServletPath()) && user == null) {
			//요청 경로가 nosessions 중에 하나이고 로그인이 안되었다면(user == null) 접근 못함.
			//contextPath로 강제 리다이렉트
			response.sendRedirect(request.getContextPath());
			return;		//필터 실행 종료
		...
		//예시 http://localhost:8081/jspBoard/updateAction.jsp?idx=183&context=테스트&title=test&page=1 가 실행되지 않도록 함.
