...
<title>우리 북카페</title>
<link rel="stylesheet"
	href="${pageContext.request.contextPath }/css/read.css?v=3">
</head>
<body>
	<main id="read">
		<h3>북챗 :: 커뮤니티</h3>
		...
		<div style="width: 80%; margin: auto; max-width: 760px;">
			<ul id="table">
				...
				<li>
					<ul class=row>
						<li>작성자</li>
						<li><c:out value="${vo.writer }" /> <span
							style="font-size: 70%; padding-left: 30px;">(<c:out
									value="${vo.ip}" />)
						</span></li>
						<li>작성날짜</li>
			...
			<form action="" method="post">
				<c:if test="${user.id == vo.writer }">
					...
					<!-- session에 저장된 user 애트리뷰트의 id와 작성자가 같으면 보이기 -->
					<!-- <a class="button" href="javascript:execute(1)">수정</a> --> <!-- GET자바스크립트 함수 : 인자값은 1은 수정 -->
					<!-- 자바스크립트 함수 :인자값 1은 수정 -->
					<!-- <a class="button" href="javascript:execute(2)">삭제</a> --> <!-- GET자바스크립트 함수 : 인자값은 2는 수정 -->
					<!-- 자바스크립트 함수 :인자값 2는 수정 -->
					<a class="button" href="javascript:formexecute(1)">수정</a> <!-- POST자바스크립트 함수 : 인자값은 1은 수정 -->
					
					<a class="button" href="javascript:formexecute(2)">삭제</a> <!-- POST자바스크립트 함수 : 인자값은 2는 수정 -->
				</c:if>
      ...
      function execute(f){
         ...
         if(yn) {
      	...
      	/* POST 요청을 위해 변경한 함수 : 가급적 url에 많은 정보를 노출하지 않으려면 post방식으로 합니다. */
	...
	<!-- 메인글 출력 끝 -->
		...
		<ul>
			...
			<li>
				<span>댓글</span>
					<span>[<c:out value="${vo.commentCount}"/>]</span> <!-- 댓글개수 -->
					</li>
			<!-- 댓글목록 -->
			...
			<li>
				<ul class="crow">
					...
					<!-- session에 저장된 user애트리뷰트의 id와 작성자의 id 가 같으면 삭제버튼 보이기  -->
